...
# Check if implementation is already complete or if checkpoint exists
set run_status [get_property STATUS $best_run]
set dcp_file "${project_file}.runs/${best_run}/${best_run}_routed.dcp"
set dcp_file2 "${project_file}.runs/${best_run}/${best_run}_placed.dcp"

# Check if we can open an existing implemented design
set can_open_design 0
if {[file exists $dcp_file]} {
    set can_open_design 1
    puts "Found routed checkpoint: $dcp_file"
} elseif {[file exists $dcp_file2]} {
    set can_open_design 1
    puts "Found placed checkpoint: $dcp_file2"
} elseif {$run_status == "write_bitstream Complete" || $run_status == "route Complete" || 
          $run_status == "place Complete" || $run_status == "phys_opt_design (Post-Route) Complete!" ||
          [string match "*Complete*" $run_status]} {
    set can_open_design 1
    puts "Implementation run status indicates completion: $run_status"
}

# Only launch implementation if absolutely necessary (shouldn't happen for hw_emu)
if {!$can_open_design && $run_status != "write_bitstream Complete" && 
    $run_status != "route Complete" && $run_status != "place Complete" && 
    $run_status != "phys_opt_design (Post-Route) Complete!"} {
    puts "WARNING: Implementation not complete. Status: $run_status"
    puts "WARNING: No checkpoint found. Cannot generate reports without implemented design."
    puts "WARNING: Please complete implementation first or ensure checkpoints exist."
    close_project
    exit 1
}
...
if {[catch {open_run $best_run} err]} {
    puts "WARNING: Could not open run $best_run: $err"
    puts "Attempting to open design checkpoint directly..."
    if {[file exists $dcp_file]} {
        if {[catch {open_checkpoint $dcp_file} err2]} {
            puts "ERROR: Failed to open checkpoint $dcp_file: $err2"
            puts "ERROR: Cannot generate reports without an implemented design"
            close_project
            exit 1
        } else {
            puts "Successfully opened checkpoint: $dcp_file"
        }
    } elseif {[file exists $dcp_file2]} {
        if {[catch {open_checkpoint $dcp_file2} err2]} {
            puts "ERROR: Failed to open checkpoint $dcp_file2: $err2"
            puts "ERROR: Cannot generate reports without an implemented design"
            close_project
            exit 1
        } else {
            puts "Successfully opened checkpoint: $dcp_file2"
        }
    } else {
        puts "ERROR: No implemented design checkpoint found and cannot open run"
        puts "ERROR: Please complete implementation first or ensure checkpoints exist"
        close_project
        exit 1
    }
}
