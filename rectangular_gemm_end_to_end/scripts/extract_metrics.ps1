...
function Find-ReportFile {
    ...
    if (Test-Path $BuildDir) {
        $buildAlt = Get-ChildItem -Path $BuildDir -Filter $name -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($buildAlt) { return $buildAlt.FullName }
    }
    return $null
}
...
# For hw_emu: Try to extract from HLS synthesis report if Vivado reports don't exist
$hlsReportPath = Find-ReportFile "dma_hls_csynth.rpt"
# Always read HLS report if it exists; individual metrics (LUTs/BRAMs/DSPs/URAM/FFs) decide
# when to fall back to HLS vs. utilization reports. This avoids N/A BRAMs when Vivado
# utilization exists but lacks Block RAM rows (common in hw_emu-only flows).
$hlsContent = if ($hlsReportPath) { Get-Content $hlsReportPath -Raw -ErrorAction SilentlyContinue } else { "" }

# Extract LUTs
$lutMatch = [regex]::Match($utilContent, "CLB LUTs\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|\s+([\d.]+)")
if (-not $lutMatch.Success -and $hlsContent) {
    # Extract from HLS report: "|Total            |      108|     0|    14861|   14063|    0|"
    # Format: |Total            |      108|     0|    14861|   14063|    0|
    $hlsLutMatch = [regex]::Match($hlsContent, "\|Total\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|")
    if ($hlsLutMatch.Success) {
        $lutUsed = $hlsLutMatch.Groups[2].Value  # LUT column (5th column)
        # Extract available from: "|Available        |     1934|  1968|  1799680|  899840|  463|"
        $hlsAvailMatch = [regex]::Match($hlsContent, "\|Available\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+)\s+\|")
        $lutTotal = if ($hlsAvailMatch.Success) { $hlsAvailMatch.Groups[1].Value } else { "899840" }
        $lutPercent = [math]::Round(([double]$lutUsed / [double]$lutTotal) * 100, 2)
    } else {
        $lutUsed = "N/A"
        $lutTotal = "N/A"
        $lutPercent = "N/A"
...
# Extract BRAMs
# Allow fractional Block RAM tile counts (e.g. 112.5 tiles) by accepting decimals.
$bramMatch = [regex]::Match($utilContent, "Block RAM Tile\s+\|\s+([\d\.]+)\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|\s+([\d.]+)")
if (-not $bramMatch.Success -and $hlsContent) {
    # Extract from HLS report: "|Total            |      108|     0|    14861|   14063|    0|"
    # Format: |Total            |      108|     0|    14861|   14063|    0|
    $hlsBramMatch = [regex]::Match($hlsContent, "\|Total\s+\|\s+(\d+)\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+\d+\s+\|")
    if ($hlsBramMatch.Success) {
        $bramUsed = $hlsBramMatch.Groups[1].Value  # BRAM_18K column (2nd column)
        # Extract available from: "|Available        |     1934|  1968|  1799680|  899840|  463|"
        $hlsAvailMatch = [regex]::Match($hlsContent, "\|Available\s+\|\s+(\d+)\s+\|")
        $bramTotal = if ($hlsAvailMatch.Success) { $hlsAvailMatch.Groups[1].Value } else { "1934" }
        $bramPercent = [math]::Round(([double]$bramUsed / [double]$bramTotal) * 100, 2)
    } else {
        $bramUsed = "N/A"
        $bramTotal = "N/A"
        $bramPercent = "N/A"
...
if (-not $dspMatch.Success -and $hlsContent) {
    # Extract from HLS report: "|Total            |      108|     0|    14861|   14063|    0|"
    # Format: |Total            |      108|     0|    14861|   14063|    0|
    $hlsDspMatch = [regex]::Match($hlsContent, "\|Total\s+\|\s+\d+\s+\|\s+(\d+)\s+\|\s+\d+\s+\|\s+\d+\s+\|")
    if ($hlsDspMatch.Success) {
        $dspUsed = $hlsDspMatch.Groups[1].Value  # DSP column (3rd column)
        # Extract available from: "|Available        |     1934|  1968|  1799680|  899840|  463|"
        $hlsAvailMatch = [regex]::Match($hlsContent, "\|Available\s+\|\s+\d+\s+\|\s+(\d+)\s+\|")
        $dspTotal = if ($hlsAvailMatch.Success) { $hlsAvailMatch.Groups[1].Value } else { "1968" }
        $dspPercent = [math]::Round(([double]$dspUsed / [double]$dspTotal) * 100, 2)
    } else {
        $dspUsed = "0"
        $dspTotal = "0"
        $dspPercent = "0.00"
...
if (-not $uramMatch.Success -and $hlsContent) {
    # Extract from HLS report: "|Total            |      108|     0|    14861|   14063|    0|"
    # Format: |Total            |      108|     0|    14861|   14063|    0|
    $hlsUramMatch = [regex]::Match($hlsContent, "\|Total\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+)")
    if ($hlsUramMatch.Success) {
        $uramUsed = $hlsUramMatch.Groups[1].Value  # URAM column (6th column)
        # Extract available from: "|Available        |     1934|  1968|  1799680|  899840|  463|"
        $hlsAvailMatch = [regex]::Match($hlsContent, "\|Available\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+)")
        $uramTotal = if ($hlsAvailMatch.Success) { $hlsAvailMatch.Groups[1].Value } else { "463" }
        $uramPercent = [math]::Round(([double]$uramUsed / [double]$uramTotal) * 100, 2)
    } else {
        $uramUsed = "0"
        $uramTotal = "0"
        $uramPercent = "0.00"
...
if (-not $ffMatch.Success -and $hlsContent) {
    # Extract from HLS report: "|Total            |      108|     0|    14861|   14063|    0|"
    # Format: |Total            |      108|     0|    14861|   14063|    0|
    $hlsFfMatch = [regex]::Match($hlsContent, "\|Total\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+)\s+\|\s+\d+\s+\|")
    if ($hlsFfMatch.Success) {
        $ffUsed = $hlsFfMatch.Groups[1].Value  # FF column (4th column)
        # Extract available from: "|Available        |     1934|  1968|  1799680|  899840|  463|"
        $hlsAvailMatch = [regex]::Match($hlsContent, "\|Available\s+\|\s+\d+\s+\|\s+\d+\s+\|\s+(\d+)\s+\|")
        $ffTotal = if ($hlsAvailMatch.Success) { $hlsAvailMatch.Groups[1].Value } else { "1799680" }
        $ffPercent = [math]::Round(([double]$ffUsed / [double]$ffTotal) * 100, 2)
    } else {
        $ffUsed = "0"
        $ffTotal = "0"
        $ffPercent = "0.00"
...
if ($timingMatch.Success) {
    ...
    # Timing status: treat WNS >= 0 as closed (no negative slack)
    if ([double]$wns -ge 0) {
...
# Read config.json to get DIM, DATA_TYPE, GEMM sizes
...
if (Test-Path $configFile) {
    try {
        ...
        # Prefer explicit GEMM_SIZE_A/AB/B; fall back to legacy GEMM_SIZE if present
        $gemmSizeA = $configData.GEMM_SIZE_A
        $gemmSizeAB = $configData.GEMM_SIZE_AB
        $gemmSizeB = $configData.GEMM_SIZE_B
        if (-not $gemmSizeA -or -not $gemmSizeAB -or -not $gemmSizeB) {
            $gemmSize = $configData.GEMM_SIZE
        }
        if ($gemmSizeA -and $gemmSizeAB -and $gemmSizeB) {
            Write-Host "  ✅ Config loaded: DIM=$dimValue, DATA_TYPE=$dataType, GEMM_SIZE_A=$gemmSizeA, GEMM_SIZE_AB=$gemmSizeAB, GEMM_SIZE_B=$gemmSizeB" -ForegroundColor Green
        } elseif ($gemmSize) {
            Write-Host "  ✅ Config loaded: DIM=$dimValue, DATA_TYPE=$dataType, GEMM_SIZE=$gemmSize" -ForegroundColor Green
        } else {
            Write-Host "  ⚠️  GEMM sizes not found in config.json, using defaults" -ForegroundColor Yellow
        }
    } catch {
...
Project: GEMM $(
    if ($gemmSizeA -and $gemmSizeAB -and $gemmSizeB) {
        "{0}x{1}x{2}" -f $gemmSizeA,$gemmSizeAB,$gemmSizeB
    } elseif ($gemmSize) {
        "{0}x{0}x{0}" -f $gemmSize
    } else {
        "xx"
    }
) (${dataType}, DIM=${dimValue}, 16 cores)
===============================================================================
...
Status: $(if ([double]$wns -ge 0) { "✅ TIMING CLOSED" } else { "❌ TIMING FAILED" })
...
if ($timingMatch.Success -and [double]$wns -ge 0) {
...
try {
    ...
    # Allow fractional Block RAM tiles in memory_utilization.txt as well.
        ...
        if (Test-Path $emuDataDir) {
            ...
            if ($matCFiles -and $matCFiles.Count -gt 0) {
                ...
                foreach ($f in $matCFiles) {
                    ...
                    # Use GEMM sizes from config if available; otherwise fall back to cubic GEMM_SIZE
                    if ($gemmSizeA -and $gemmSizeAB -and $gemmSizeB) {
                        $M = [int]$gemmSizeA; $K = [int]$gemmSizeAB; $N = [int]$gemmSizeB
                    } else {
