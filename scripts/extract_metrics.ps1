...
$aiePowerMatch = [regex]::Match($powerContent, "AIE2\s+\|\s+([\d.]+)\s+\|\s+(\d+)\s+\|\s+(\d+)\s+\|\s+([\d.]+)")
...
if ($aiePowerMatch.Success) {
    ...
    # Read actual core count from active_cores.json
    $activeCoresFile = "logs\active_cores.json"
    $aieCoresUsed = "0"
    $aieCoresTotal = "34"  # Total AI Engine cores available on platform
    
    if (Test-Path $activeCoresFile) {
        try {
            $activeCoresData = Get-Content $activeCoresFile | ConvertFrom-Json
            $aieCoresUsed = $activeCoresData.ActiveCores.Count
            ...
            Write-Host "  Source: active_cores.json (precise count)" -ForegroundColor Green
            Write-Host "  Note: Vivado reports AI Engine as 1 IP block, not individual cores" -ForegroundColor Yellow
        } catch {
            Write-Host "  AI Engine Power: $aiePower W" -ForegroundColor White
            Write-Host "  AI Engine Cores: Error reading active_cores.json" -ForegroundColor Red
            Write-Host "  Fallback: Using calculated value (SPLIT × CASC_LN = 2 × 8 = 16)" -ForegroundColor Yellow
            $aieCoresUsed = "16"
            $aieUtilPercent = "47.06"
        }
    } else {
        Write-Host "  AI Engine Power: $aiePower W" -ForegroundColor White
        Write-Host "  AI Engine Cores: active_cores.json not found" -ForegroundColor Yellow
        Write-Host "  Fallback: Using calculated value (SPLIT × CASC_LN = 2 × 8 = 16)" -ForegroundColor Yellow
        $aieCoresUsed = "16"
        $aieUtilPercent = "47.06"
    }
...
Project: GEMM 32x32x32 (int16, 16 cores)
===============================================================================
