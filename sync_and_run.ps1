...
function Load-Config {
    ...
    if (Test-Path $script:configPath) {
        try {
            ...
            # Note: WRD_LN, SUB_TILE_M, SUB_TILE_K, SUB_TILE_N are now calculated from DATA_TYPE
...
function Save-Config {
    $obj = [ordered]@{
        ...
        # Note: WRD_LN, SUB_TILE_*, GRAPH_ITER_CNT are calculated parameters
        # They should not be saved to config.json as they are derived from other parameters
    }
...
function Ensure-DerivedParams {
    ...
    if ($script:WRD_LN -ne $newWRD) { $script:WRD_LN = $newWRD; $needSave = $true }
    $st = Get-SubTileSizes -dt $script:dataType
    if ($script:SUB_TILE_M -ne $st.M) { $script:SUB_TILE_M = $st.M; $needSave = $true }
    if ($script:SUB_TILE_K -ne $st.K) { $script:SUB_TILE_K = $st.K; $needSave = $true }
    if ($script:SUB_TILE_N -ne $st.N) { $script:SUB_TILE_N = $st.N; $needSave = $true }
...
"@
    ...
    $vivadoReportsPattern = "$remoteReportsRepo/gemm_${script:gemmSize}x${script:gemmSize}x${script:gemmSize}/x1/*.txt"
    $vivadoReportsPattern2 = "$remoteReportsRepo/gemm_${script:gemmSize}x${script:gemmSize}x${script:gemmSize}/x1/*.html"
    $vivadoReportsPattern3 = "$remoteReportsRepo/gemm_${script:gemmSize}x${script:gemmSize}x${script:gemmSize}/x1/*.xml"
    
    # Copy text reports (utilization, timing, power, etc.)
    Copy-RemoteFilesIfExist -RemotePattern $vivadoReportsPattern -LocalDir $localReportsDir -Label "Vivado text reports"
    
    # Copy HTML reports
    Copy-RemoteFilesIfExist -RemotePattern $vivadoReportsPattern2 -LocalDir $localReportsDir -Label "Vivado HTML reports"
    
    # Copy XML reports
    Copy-RemoteFilesIfExist -RemotePattern $vivadoReportsPattern3 -LocalDir $localReportsDir -Label "Vivado XML reports"
...
function Extract-Metrics {
    ...
    # Check if reports directory exists
    $reportsDir = "reports\gemm_$($script:gemmSize)x$($script:gemmSize)x$($script:gemmSize)\x1"
    if (-not (Test-Path $reportsDir)) {
        Write-Host "❌ Reports directory not found: $reportsDir" -ForegroundColor Red
        Write-Host "Please run option 5.9 (Sync Vivado reports) first to download reports from remote server." -ForegroundColor Yellow
...
while ($true) {
    ...
    Write-Host "   5.5 Sync comprehensive logs and reports"
