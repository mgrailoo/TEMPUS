...
def main():
    ...
    if DIM_AB_orig != DIM_AB:
        print(f"\n[WARNING] DIM_AB was set to {DIM_AB_orig} in config, but was automatically corrected to {DIM_AB}")
        print(f"         Expected DIM_AB <= GEMM_SIZE_AB / CASC_LN_AB = {GEMM_SIZE_AB} / {CASC_LN_AB} = {expected_dim_ab}")
        print(f"         Effective DIM_AB = min({DIM_AB_orig}, {expected_dim_ab}) = {DIM_AB}")
