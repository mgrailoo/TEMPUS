...
def read_all_parameters_from_config(config_file_path):
    ...
    try:
        ...
        v = config_data.get('SUB_TILE_M')
        if v is not None: 
            params['SUB_TILE_M'] = int(v)
        else:
            sub_tile_m, _, _ = get_optimal_sub_tile_size(params['DATA_TYPE'])
            params['SUB_TILE_M'] = sub_tile_m
            
        v = config_data.get('SUB_TILE_K')
        if v is not None: 
            params['SUB_TILE_K'] = int(v)
        else:
            _, sub_tile_k, _ = get_optimal_sub_tile_size(params['DATA_TYPE'])
            params['SUB_TILE_K'] = sub_tile_k
            
        v = config_data.get('SUB_TILE_N')
        if v is not None: 
            params['SUB_TILE_N'] = int(v)
        else:
            _, _, sub_tile_n = get_optimal_sub_tile_size(params['DATA_TYPE'])
            params['SUB_TILE_N'] = sub_tile_n
...
def generate_split_output_files(matrix_C, split, dim, wrd_ln, path, data_type, sub_tile_m=None, sub_tile_n=None):
    """
    ...
    if sub_tile_m is None or sub_tile_n is None:
...
def write_matrix_A_cascade(matrix_A, casc_idx, filename, gemm_size, split, casc_ln, dim, wrd_ln, data_type, sub_tile_m=None, sub_tile_k=None):
    ...
    try:
        ...
        # Use provided sub-tile values or fallback to calculated values
        if sub_tile_m is None or sub_tile_k is None:
...
def write_matrix_B_split_cascade(matrix_B, split_idx, casc_idx, filename, gemm_size, split_val, casc_ln, dim, wrd_ln, data_type, sub_tile_k=None, sub_tile_n=None):
    ...
    try:
        ...
        # Use provided sub-tile values or fallback to calculated values
        if sub_tile_k is None or sub_tile_n is None:
...
def generate_cascade_input_files(matrix_A, matrix_B, split, casc_ln, dim, wrd_ln, path, data_type, sub_tile_m=None, sub_tile_k=None, sub_tile_n=None):
    """
    ...
    # Use provided sub-tile values or fallback to calculated values
    if sub_tile_m is None or sub_tile_k is None or sub_tile_n is None:
        sub_tile_m, sub_tile_k, sub_tile_n = get_optimal_sub_tile_size(data_type)
    sub_tiles_per_row = max(1, dim // sub_tile_m)
    sub_tiles_per_col = max(1, dim // sub_tile_n)
    ...
    for casc in range(casc_ln):
        ...
        success = write_matrix_A_cascade(matrix_A, casc, filename, rows_A, split, casc_ln, dim, wrd_ln, data_type, sub_tile_m, sub_tile_k)
        if not success:
    ...
    for casc in range(casc_ln):
        ...
        success = write_matrix_B_split_cascade(matrix_B, 0, casc, filename, rows_B, split, casc_ln, dim, wrd_ln, data_type, sub_tile_k, sub_tile_n)
        if not success:
            print(f"Failed to generate {filename}")
    ...
    for casc in range(casc_ln):
        ...
        success = write_matrix_B_split_cascade(matrix_B, 1, casc, filename, rows_B, split, casc_ln, dim, wrd_ln, data_type, sub_tile_k, sub_tile_n)
        if not success:
            print(f"Failed to generate {filename}")
...
def list_valid_tp_dims(gemm_size, dim, split, casc_ln, data_type, sub_tile_m=None, sub_tile_k=None, sub_tile_n=None):
    # Use provided sub-tile values or fallback to calculated values
    if sub_tile_m is None or sub_tile_k is None or sub_tile_n is None:
        sub_m, sub_k, sub_n = get_optimal_sub_tile_size(data_type)
    else:
        sub_m, sub_k, sub_n = sub_tile_m, sub_tile_k, sub_tile_n
